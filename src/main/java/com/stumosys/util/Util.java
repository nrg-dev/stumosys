...
public class Util {
	...
	public static String getStudentRollNumber(String name) {
		...
		try {
			...
			/*
			 * logger.debug("Year -->"+result[0]); logger.debug(
			 * "Month -->"+result[1]); logger.debug("Date -->"+result[2]);
			 * logger.debug("Hours -->"+result[3]); logger.debug(
			 * "Min -->"+result[4]); logger.debug("Seconds -->"
			 * +result[5]); logger.debug("Mode -->"+result[6]);
			 * logger.debug("Zone -->"+result[7]);
			 */

			rollNumber = result[0] + name.charAt(0) + a[Integer.valueOf(result[5])][1] + name.length()
	 ...
	 *         return  session.getAttribute("username").toString();       }
	 *               public static String getUserId()       {
	 *         HttpSession session = getSession(); if ( session != null ) return
	 * (String) session.getAttribute("userid");         else             return
	 * null; }
	 */
}
